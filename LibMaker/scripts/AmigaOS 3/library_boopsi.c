...
__saveds struct Library *LibInit(struct Library *unused, APTR seglist, struct Library *sysb);
__saveds struct MyLibBase *lib_init(struct MyLibBase *base, APTR seglist, struct Library *SysBase);
APTR lib_expunge(struct MyLibBase *base);
__saveds struct Library *LibOpen(void);
__saveds ULONG LibClose(void);
__saveds APTR LibExpunge(void);
