...
multiline['EXTRABASEFIELDS'] = function(indent)
	local s = ''
	if spec.altivec then s = s .. indent .. 'BOOL                    HaveAltiVec;\n' end
	return s
end
...
multiline['INITALTIVECCHECK'] = function(indent)
	local s = ''
	if spec.altivec then s = indent .. 'CheckAltiVec(base);\n' end
	return s
end
...
multiline['JUMPTABLE'] = function(indent)
	...
	if #spec.functions > 0 then
		s = s .. indent .. '(APTR)FUNCARRAY_32BIT_SYSTEMV,\n'
		for func = 1, #spec.functions do
			s = s .. indent .. '(APTR)Lib' .. spec.functions[func].name .. ',\n'
		end
		s = s .. indent .. '(APTR)0xFFFFFFFF,\n'
	end
	return s
end
...
multiline['AVCHECKROUTINE'] = function(ident)
	local s
	if not spec.altivec then return '' end
	s = 'void CheckAltiVec(struct MyLibBase *base)\n{\n'
	s = s .. '\tstruct Resident *mos;\n\n\tif ((mos = FindResident("MorphOS")))\n\t{\n'
	s = s .. '\t\tif (mos->rt_Flags & RTF_EXTENDED)\n\t\t{\n'
	s = s .. '\t\t\tif ((mos->rt_Version > 1) || ((mos->rt_Version == 1) && (mos->rt_Revision > 4)))\n'
	s = s .. '\t\t\t{\n\t\t\t\tLONG have_altivec = 0;\n\n'
	s = s .. '\t\t\t\tNewGetSystemAttrs(&have_altivec, sizeof(ULONG), SYSTEMINFOTYPE_PPC_ALTIVEC, NULL);\n'
	s = s .. '\t\t\t\tif (have_altivec) base->HaveAltiVec = TRUE;\n\t\t\t}\n\t\t}\n\t}\n\n\treturn;\n}\n\n\n'
	return s
end
...
function gen_library_function(fspec, separated)
	...
	proto = fspec.type .. ' Lib' .. fspec.name .. '(struct MyLibBase *base'
...
------------------------------------------------------------------------------------------------
-- Still does not support variable include path when creating directories...
...
function generate_includes()
	...
	incdef = 'CLIB_' .. incdef .. '_PROTOS_H'
	fh:write('#ifndef ' .. incdef .. '\n#define ' .. incdef .. '\n\n')
