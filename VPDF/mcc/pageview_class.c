...
static void rendermarker(struct Data *data, struct RastPort *rp, struct Window *win, struct markernode *mn, int x0, int y0)
{
	...
	#ifdef __amigaos4__
	ProcessPixelArray(rp, x0 + x1 - 1, y0 + y2 - 1, x2 - x1, y1 - y2, dri,PPA_Darken,10,TAG_END);
		...
		if (data->image && data->image->data)
		{
			...
			if (data->quiet == FALSE)
			{
				...
				if (!ISLISTEMPTY(&data->markerlist))
				{
					...
					{
						#ifdef __amigaos4__
						rendermarker(data, _rp(obj), _win(obj), mn, x0, y0);
						#else
				...
				{
					#ifdef __amigaos4__
					rendermarker(data, _rp(obj), _win(obj), &data->lasso.marker, x0, y0);	
					#else
