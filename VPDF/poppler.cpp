...
#ifdef __amigaos4__
// works and __init_poppler(void) and __init_popler_main(void). Both runs before
void __init_poppler(void)__attribute__((constructor));
void __init_poppler(void)
{
    //printf("we in __init_poppler\n");
   	D(kprintf("we in CONSTRUCTOR_P\n"));
	InitSemaphore(&semaphore);
	setErrorCallback(pdfErrorFunction, NULL);
}

void __exit_poppler(void)__attribute__((destructor));
void __exit_poppler(void)
{
	//printf("destroy poppler:%p\n", &semaphore);
}
#else
...
#ifdef __amigaos4__				
                char *cstr;
                cstr = CodesetsConvertStr(CSA_SourceMIBenum, CS_MIBENUM_UTF_16,
                          CSA_DestMIBenum, CS_MIBENUM_SYSTEM,
						  CSA_SourceLen, length * 2 - 2 ,
                          CSA_Source, (unsigned char*)string + 2,
						  CSA_MapForeignChars, TRUE,
                          TAG_DONE);
                if(cstr != NULL)
                 {
						strncpy(buff, cstr, length + 1);
						buff[length] = 0;
...
static char *convertUTF32ToANSI(Unicode *string, int length)
{
		...
		//for(int x=0;x<length;x++) printf("string[%d]=%x\n",x,string[x]);
		
#ifdef __amigaos4__
		
                char *cstr;
				
                cstr = CodesetsConvertStr(CSA_SourceMIBenum, CS_MIBENUM_UTF_32,
                                          CSA_DestMIBenum, CS_MIBENUM_SYSTEM,
                                          CSA_Source, (unsigned char *)string,
										  CSA_SourceLen, length * 4,
										  CSA_MapForeignChars, TRUE,
                                          TAG_DONE);
                if(cstr != NULL)
                 {
...
#ifndef USE_SPLASH
	D(kprintf("Cairo Base:%p. sema:%p, %p, %p\n", CairoBase, &semaphore, foo, &foo);)
#endif
	...
	if(l != NULL)
	{
		...
		if(items != NULL)
		{
			...
			for(i = 0; i < items->getLength(); i++)
			{
				...
				//	printf("n->item->getTitleLength() = %d\n",n->item->getTitleLength());					
				//	printf("n->item->getTitle() = %x\n",*(n->item->getTitle()));
...
static struct searchresult *nextsearchresult(struct devicecontext *ctx, int direction)
{
	...
	{
		#ifdef __amigaos4__
		return (struct searchresult*)GetSucc((Node *)ctx->search.currentsearchresult);
		#else
	...
	{
		#ifdef __amigaos4__
		return (struct searchresult*)GetPred((Node *)ctx->search.currentsearchresult);
		#else
...
int pdfSearch(void *_ctx, int *page, char *phrase, int direction, double *x1, double *y1, double *x2, double *y2)
{
	...
	if((ctx->search.phrase == NULL || 0 == strcmp(ctx->search.phrase, phrase)) && ctx->search.page == *page && (ctx->search.currentsearchresult != NULL && nextsearchresult(ctx, direction) == NULL))
	{
		...
		{
			#ifdef __amigaos4__
			struct searchresult *srn = (struct searchresult*)GetHead((List *)&ctx->search.searchresultlist);
			#else
	...
	else if(ctx->search.phrase == NULL || 0 != strcmp(ctx->search.phrase, phrase) || ctx->search.page != *page)
	{
		...
		if (found)
		{
			...
			if(direction > 0) {
				#ifdef __amigaos4__
				ctx->search.currentsearchresult = (struct searchresult*)GetHead((List *)&ctx->search.searchresultlist);
				#else
				...
				#ifdef __amigaos4__
				ctx->search.currentsearchresult = (struct searchresult*)GetTail((List *)&ctx->search.searchresultlist);
				#else
