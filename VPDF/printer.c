...
DEFMMETHOD(VPDFPrinter_PrinterPrefs)
{
	...
	if (xget(data->chkPS, MUIA_Selected))
		#ifdef __amigaos4__
		SystemTagList("TURBOPRINT:TurboPrefs", TAG_DONE);
		#else
...
{
	GETDATA;
	MUI_Request(_app(data->sldFirst), _window(obj), 0, "VPDF  Print Document Error", "OK", "Failed to initialize output device.\nTurboPrint is required for printing", NULL);
	return TRUE;
}
